                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
                            {pieces.map((piece, i) => (
                                <PieceCard
                                    ...
                                    index={i}
                                    series={meta}
                                    onOpen={() => setSelected({ series: n, piece: i })}
                                />
                            ))}
                        </div>
                    </section>
...
// Grid tile. Mouse hover previews the title and the start of the story;
// clicking (or tapping, or Enter) opens the full detail view.
function PieceCard({
    piece,
    index,
    series,
    ...
    return (
        ...
        >
            <button
                ...
                aria-label={`Open ${alt}`}
                className="relative block w-full overflow-hidden cursor-zoom-in text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#f2dcc0]"
            >
                <motion.div animate={{ scale: hover ? 1.04 : 1 }} transition={{ duration: 0.5, ease: 'easeOut' }}>
                    <Image
                ...
                <AnimatePresence>
                    {hover && (title || story) && (
                        <motion.span
                            className="absolute inset-x-0 bottom-0 block p-4 md:p-6 pt-16 bg-gradient-to-t from-black via-black/85 to-transparent"
                        ...
                        >
                            ...
                            {title && (
                                <span className="block text-base md:text-lg font-semibold tracking-tight">
                                    {number && <span className="font-mono text-xs mr-2 text-[#e6e1d6]/50">{number}</span>}
                                    {title}
                                </span>
                            )}
                            {story && (
                                <span className="mt-1 text-sm md:text-base leading-relaxed text-[#e6e1d6]/85 line-clamp-3 whitespace-pre-line">
                                    {story}
                                </span>
                            )}
                            <span
